...
const loginSchema = Joi.object({
    ...
    role: Joi.string().valid('sales_role', 'sales_appointment_setter').required(),
    fcm_token: Joi.string().allow(null, '')
});

// Login handler
async function loginSales(req, res) {
    try {
        let { email, password, role, fcm_token } = req.body;
...
// Logout handler
async function logoutSales(req, res) {
    try {
        const user = await Users.findOne({
            where: { 
                id: req.userId,
                role_name: ['sales_role', 'sales_sales_appointment_setter']
            },
